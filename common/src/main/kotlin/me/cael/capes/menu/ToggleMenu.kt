...
class ToggleMenu(parent: Screen, gameOptions: GameOptions) : MainMenu(parent, gameOptions) {
    ...
    override fun init() {
        ...
        addDrawableChild(ButtonWidget.builder(CapeType.MOCCAPES.getToggleText(config.enableMocCapes)) {
            config.enableMocCapes = !config.enableMocCapes
            config.save()
            it.message = CapeType.MOCCAPES.getToggleText(config.enableMocCapes)
